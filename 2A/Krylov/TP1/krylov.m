...
while (j <= maxit && relres > tol) % critère d'arrêt
    ...
    if(type == 0)
        ...
        y = H(1:j,1:j)\beta_e1;
        norme_estime = H(j+1,j)*abs(y(end));
    else
        % GMRES
        ...
        %y = H(1:j+1,1:j)\beta_e1;
        [C,R] = qr(sparse(H(1:j+1,1:j)),beta_e1);
        y = R(1:j,1:j)\C(1:j);
        norme_estime = abs(C(j+1));
    end
    ...
    %norm(norme_estime - resvec(j+1))
    resvec(j+1) = norme_estime;
